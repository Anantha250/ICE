...
analogWrite(enB, 0); // Speed Control
    ...
    countBlink = 1;
      ...
      //เช็คว่า มีการเริ่มรับกระพริบ และเวลาที่ผ่านไปน้อยกว่าหรือเท่ากับ 5000 มิลลิไหม
  while(millis() - lastBlink <= 5000 && countBlink == 1) {
    if (analogRead(ldrPin) < 500) {
      ...
      //ตรงนี้เอาไว้สั่งเลี้ยว
  if (countBlink != 0 && turnyoung == false) {
    if (countBlink == 1) {
      forward(200);
      turnLeft(450); //โค้ดเลี้ยวซ้าย จะเลี้ยวเท่าไหร่ก็แล้วแต่
      turnyoung = true;
      countBlink = 0 ;
       spceial = false;
       state = 1;
      ...
      forward(200);
      turnRight(500); //โค้ดเลี้ยวขวา จะเลี้ยวเท่าไหร่ก็แล้วแต่
      forward(950);
      turnLeft(380);
      turnyoung = true;
      countBlink = 0 ;
...
analogWrite(enB, 0); // Speed Control
  ...
  if (countBlink != 0 && turnyoung == false) {
    if (countBlink == 1) {
      ...
      forward(200);
      turnRight(450); //โค้ดเลี้ยวขวา จะเลี้ยวเท่าไหร่ก็แล้วแต่
      turnyoung = true;
      countBlink = 0 ;
      spceial = false;
      state = 3;
